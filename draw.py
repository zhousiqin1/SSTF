...
def plot_confusion(labels, predicted, save_dir="./fig", experiment_name=""):
    ...
    sns.heatmap(cm, annot=True, cbar=True, cmap='Blues', fmt='.2f', xticklabels=class_names, yticklabels=class_names, ax=ax)
    # 设置 xticklabels 和 yticklabels 的转向
...
def plot_confusion_raw(labels, predicted, save_dir="./fig", experiment_name=""):
    # 1. 计算混淆矩阵 (注意：这里去掉了 normalize='true'，默认就是原始个数)
...
def plot_train_val_loss(train_losses, val_losses, save_dir="./fig"):
    ...
    plt.axvline(x=min_val_epoch, color='g', linestyle='--', alpha=0.7, label=f'Best Val Loss (Epoch {min_val_epoch})')
...
def plot_train_val_accuracy(train_accs, val_accs, save_dir="./fig"):
    ...
    plt.axvline(x=max_val_epoch, color='g', linestyle='--', alpha=0.7, label=f'Best Val Acc (Epoch {max_val_epoch})')
...
def plot_model_performance_summary(train_losses, train_accs, val_losses, val_accs, test_acc, predicted, labels, save_dir="./fig"):
    """绘制模型性能总结"""
